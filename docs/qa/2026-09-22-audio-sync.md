...
## Separate text-only language guard

A following commit adds explicit Dutch/Polish audio unavailability, independent of the translation owner's future `lib/languages.ts` catalog expansion. Existing language availability is preserved. Checkout, generation (including an omitted `lang` resolved from the book), and scoped preview refuse with 422 / `AUDIOBOOK_LANGUAGE_UNAVAILABLE` before Stripe checkout/redemption, queueing or provider synthesis. The Audio panel explains text-only status and disables preview/generation/payment; this is a capability limit, not a premium gate. Existing already-generated audio is not removed.

Guard UI QA: (1) open `/dev/audiobook-preview`, (2) select Dutch and Polish and read the text-only notice, (3) verify disabled preview/generation and no payment CTA, (4) return to Swedish and verify existing enabled actions, (5) repeat at 390px. `scripts/qa-audio-language.mjs` automates those local fixture checks. Integration must include this guard before the new text catalog is exposed. No NL/PL provider run or production flag change was performed.
