    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-4 text-xs text-muted-foreground">
        <label className="flex items-center gap-2">
          ...
          }} className="min-h-11 rounded-lg border border-border bg-background px-2 py-1.5 text-foreground">
      ...
      {failedSrc === src && (
        <div role="alert" className="text-xs text-destructive">
          <p>Could not play this audio. Check your connection and try again.</p>
          <button type="button" className="mt-2 min-h-11 rounded-lg border border-border px-3 text-foreground" onClick={() => {
            setFailedSrc(null);
            if (onRetry) onRetry();
