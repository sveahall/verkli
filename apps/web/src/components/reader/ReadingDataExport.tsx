...
export default function ReadingDataExport() {
  ...
  async function download() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(response.status === 401 ? "Please sign in again to download your data." : response.status === 413 ? "Your reading data exceeds 10,000 entries in a list. Please contact support for a larger reading-data export. No partial file was downloaded." : "Could not export your reading data. Please try again.");
      }
  ...
  return (
    <section className="mt-8 space-y-3 border-t border-border pt-6" aria-labelledby="reading-data-heading">
      ...
      <p className="text-sm text-muted-foreground">
        ...
        This includes empty lists if you have not started reading yet. Purchases, messages, highlights and manuscripts are not included.
        Each list supports up to 10,000 entries; larger exports return an error. Changes made during the export may not be included.
      </p>
