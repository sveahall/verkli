...
export default function PricingPageContent({ annualAvailable }: { annualAvailable: boolean }) {
  ...
  // Derived rather than stored: if the annual row goes away, a selection made
  // before it did cannot keep showing a price nobody can buy.
  ...
  return (
    <main className="author-light relative min-h-screen bg-background text-foreground -mt-[88px]">
      ...
      <section className="relative isolate flex flex-col items-center overflow-hidden px-6 pb-0 pt-[148px] text-center">
        ...
        {/* Annual toggle. Only rendered when the catalog holds an annual price:
            advertising a period that checkout cannot sell is the same gap the
            toggle itself had before the interval reached checkout. */}
        {annualAvailable && (
      ...
      <section className="mx-auto mt-24 w-full max-w-[680px] px-6">
        ...
        <div>
          {FAQS.filter((item) => annualAvailable || !item.annualOnly).map((item, i) => (
