...
export default function TranslationQualityPreview() {
  const [scenario, setScenario] = useState<"pass" | "issues" | "unavailable" | "missing-language">("issues");
  ...
  const request = useCallback<typeof fetch>(async (input, init) => {
    ...
    if (scenario === "missing-language" && (url.pathname.endsWith("/translation-preview") || url.pathname.endsWith("/translate"))) {
      return Response.json({ error: "SOURCE_LANGUAGE_MISSING" }, { status: 422 });
    }
  ...
  return <main className="mx-auto max-w-6xl px-5 py-12 text-foreground sm:px-10">
    ...
    <div className="my-8 flex flex-wrap items-center gap-3">
      {([['issues', 'Unresolved issue'], ['pass', 'Checks passed'], ['unavailable', 'Provider unavailable'], ['missing-language', 'Missing source language']] as const).map(([value, label]) => <button key={value} type="button" aria-pressed={scenario === value} onClick={() => setScenario(value)} className={`rounded-full border border-border px-4 py-2 text-sm ${scenario === value ? 'bg-primary text-primary-foreground' : 'bg-card'}`}>{label}</button>)}
