...
function PreparedAnalysis({ mode }: { mode: string }) {
  ...
  const job = useRef<BookAnalysisResult | null>(null);
  const request = useMemo<typeof fetch>(() => async (_input, init) => {
    const body = init?.body ? JSON.parse(String(init.body)) : null;
    if (!body) return Response.json({ analysis: job.current });
    if (mode === "budget") return Response.json({ error: "This analysis exceeds your remaining daily editorial AI allowance. The analysis is incomplete." }, { status: 429 });
    ...
    if (body.action === "start") {
      job.current = { jobId: "44444444-4444-4444-8444-444444444444", status: "pending", createdAt: "2026-09-17T12:00:00Z",
      ...
      if (mode === "failure" && job.current.completedParts === 1) { job.current.status = "failed"; job.current.error = "The AI response could not be verified. No complete report was produced."; return Response.json({ error: job.current.error }, { status: 502 }); }
      if (job.current.completedParts === 3) { job.current.status = "completed"; job.current.report = fixtureReport; }
      else job.current.completedParts += 1;
    }
    return Response.json({ analysis: job.current });
  }, [mode]);
  ...
  return <><div className="my-4 flex flex-wrap gap-3"><button className="min-h-11 rounded-full border border-border px-4 text-sm" onClick={() => setEdited((value) => !value)}>Change manuscript</button>
    ...
    <WholeBookAnalysisPanel bookId={bookId} versionId={versionId} chapters={mode === "empty" ? [] : chapters} saveBlocked={mode === "conflict"} request={request} />
  </>;
}
export default function BookAnalysisPreview() {
  ...
  return <main className="mx-auto max-w-6xl px-4 py-8 sm:px-8"><div className="flex flex-wrap items-center justify-between gap-4 text-sm text-muted-foreground">
    ...
    <div className="flex flex-wrap gap-3"><label className="flex items-center gap-2">State<select className="min-h-11 rounded-xl border border-border bg-card px-3 text-base" value={mode} onChange={(event) => setMode(event.target.value)}>{["ready", "slow", "failure", "budget", "unavailable", "empty", "conflict"].map((value) => <option key={value}>{value}</option>)}</select></label>
      </div>
