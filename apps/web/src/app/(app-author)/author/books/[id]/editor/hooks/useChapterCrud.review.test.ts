...
const { assertReviewCanApply, persistReviewedChapterContent } = await import("./useChapterCrud.review");
...
describe("review compare-and-swap persistence", () => {
  let row: { id: string; book_id: string; content: string | null; updated_at: string | null };
  ...
  beforeEach(() => {
    row = { id: "chapter-1", book_id: "book-1", content: "old", updated_at: "2026-09-14T12:00:00.123456+00:00" };
    readFilters = [];
    writeFilters = [];
    writes = 0;
    readError = null;
      ...
      from: (table: string) => {
        ...
        return {
          select: (fields: string) => {
            expect(fields).toBe("id, content, updated_at");
            const query = {
  ...
  it("checks source content then saves only the unchanged book/chapter/timestamp", async () => {
    ...
    expect(writeFilters).toEqual([["book_id", "book-1"], ["id", "chapter-1"], ["updated_at", "2026-09-14T12:00:00.123456+00:00"]]);
  });
  it.each([
