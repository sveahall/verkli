...
export type ApplyReview = (chapterId: string, expectedContent: string | null, nextContent: Record<string, unknown>) => Promise<void>;
type Props = {
  bookId: string;
  chapters: Chapter[];
...
const buttonClass = "rounded-lg border border-border px-3 py-2 text-sm font-medium disabled:cursor-not-allowed disabled:opacity-50 hover:bg-muted";
...
export default function EditorialReviewPanel({ bookId, chapters, activeVersionId, bookVersions, onApplyReview, saveBlocked }: Props) {
  ...
  async function run(all: boolean) {
    ...
    const targets = all ? nonempty : nonempty.filter((chapter) => chapter.id === chapterId);
    ...
    try {
        ...
        for (let part = 0; part < partCount; part += 1) {
          ...
          if (!response.ok) {
            ...
            throw new Error(response.status === 403
              ? "The review could not be sent from this page. Refresh and try again."
              : resolveErrorMessage(code, code || "The review could not be completed."));
          }
      ...
      setProgress(`Review complete: ${finished} of ${targets.length} chapters, including every text part.${available.length > nonempty.length && all ? ` ${available.length - nonempty.length} empty chapters skipped.` : ""}`);
    } catch (cause) {
  ...
  async function accept(result: ReviewResult, index: number) {
    ...
    try {
      ...
      await onApplyReview(result.chapterId, before, next);
  ...
  async function undoLast() {
    ...
    setApplying(true); setError(null);
    try {
      await onApplyReview(undo.chapterId, undo.after, JSON.parse(undo.before));
      expected.current.set(undo.chapterId, undo.before);
  ...
  return (
    <section aria-label="Editorial review" className="min-w-0 space-y-4 [overflow-wrap:anywhere] rounded-2xl border border-border bg-card p-5">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">Spelling, grammar and awkward phrasing are flagged on the passage itself. When both readers are available, a note they both mark says so. You choose which changes to accept. Download your report before leaving this page; review decisions are kept for this session.</p>
      </div>
      {!nonempty.length ? <p className="text-sm text-muted-foreground">Add and save a chapter before starting a review.</p> : <>
        <div className="flex flex-wrap gap-3">
          ...
          <label className="grid min-w-0 max-w-full gap-1 text-sm">Chapter<select className={`${buttonClass} min-w-0 max-w-full`} value={chapterId} disabled={disabled} onChange={(event) => setChapterId(event.target.value)}>{nonempty.map((chapter, index) => <option key={chapter.id} value={chapter.id}>{index + 1}. {chapter.title}</option>)}</select></label>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {mode !== "translation" && <button className={buttonClass} disabled={disabled} onClick={() => void run(true)}>Review all {nonempty.length} chapters</button>}
      ...
      {results.map((result) => <article key={keyFor(result)} className="space-y-3 border-t border-border pt-4">
