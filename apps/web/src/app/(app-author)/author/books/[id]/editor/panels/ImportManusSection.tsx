...
export default function ImportManusSection({
  ...
  const handleFile = useCallback((file: File | null) => {
    ...
    if (!IMPORT_ALLOWED_EXT.includes(extension)) {
  ...
  const runChapterRepair = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/books/${bookId}/chapters/repair`, {
        method: "POST",
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <div className="space-y-4 rounded-2xl border border-black/[0.05] bg-white/60 p-5 shadow-[0_1px_3px_rgba(0,0,0,0.02)] backdrop-blur-sm dark:border-white/[0.06] dark:bg-white/[0.02] dark:shadow-none">
        ...
        <div className="flex items-center gap-3">
          ...
          />
          <label htmlFor="import-new-version" className="text-sm text-slate-700 dark:text-white/80">
        ...
        <div className="flex items-center gap-3">
          <input
            ...
            className="h-4 w-4 accent-[#907AFF]"
          />
          <label htmlFor="import-overwrite" className="text-sm text-slate-700 dark:text-white/80">
            Overwrite draft
          </label>
        </div>
        {overwrite && (
      ...
      <div
        className={`rounded-2xl border-2 border-dashed p-8 text-center transition-colors ${
          selectedFile ? "border-[#907AFF]/40 bg-[#907AFF]/5 dark:bg-[#907AFF]/10" : "border-black/[0.06] bg-white/50 dark:border-white/[0.06] dark:bg-white/[0.02]"
        }`}
      ...
      >
        <input
          ...
          disabled={uploading}
        />
        <p className="mb-2 text-sm text-slate-600 dark:text-white/60">
      ...
      <button
        ...
        className="rounded-xl bg-slate-900 px-4 py-2.5 text-[13px] font-semibold text-white shadow-sm transition-all hover:bg-slate-800 hover:shadow-md disabled:cursor-not-allowed disabled:opacity-50 dark:bg-white dark:text-slate-900"
      >
      ...
      {lastResult && !importJobs.some((job) => isJobActiveStatus(job.status)) && (
        <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 dark:border-emerald-800 dark:bg-emerald-950/30">
          ...
          {lastResult.chaptersCreated != null && (
            <p className="text-sm text-emerald-700 dark:text-emerald-300">Chapter count: {lastResult.chaptersCreated}</p>
          )}
        </div>
      )}
