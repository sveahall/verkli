    ...
    <div className="mx-auto max-w-2xl py-6">
      ...
      <div className="mt-10 grid grid-cols-3 gap-6 text-center">
        {[
          ...
          <div key={item.title}>
            ...
            <p className="mt-2 text-[13px] font-medium text-foreground dark:text-foreground">{item.title}</p>
          </div>
        ))}
      </div>

      <div className="mt-12">
        <div className="grid grid-cols-2 gap-px overflow-hidden rounded-2xl bg-muted ring-1 ring-black/[0.04] dark:bg-card dark:ring-white/10">
          {(["softcover", "hardcover"] as const).map((format) => {
            const info = FORMAT_INFO[format];
            return (
              <div key={format} className="bg-card px-6 py-6 dark:bg-card">
                <p className="text-[13px] font-semibold text-foreground dark:text-foreground">{info.label}</p>
                <p className="mt-0.5 text-xs text-muted-foreground dark:text-muted-foreground">{info.tagline}</p>
                <div className="mt-5 space-y-2.5 text-[13px]">
                  <div className="flex items-center justify-between">
                    <span className="text-muted-foreground dark:text-muted-foreground">Production</span>
                    <span className="font-medium text-foreground dark:text-foreground">{info.cost}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-muted-foreground dark:text-muted-foreground">Shipping SE</span>
                    <span className="font-medium text-foreground dark:text-foreground">{info.shippingSE}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-muted-foreground dark:text-muted-foreground">Shipping EU</span>
                    <span className="font-medium text-foreground dark:text-foreground">{info.shippingEU}</span>
                  </div>
                </div>
              </div>
            );
      ...
      <div className="mt-12 text-center">
        <button
          ...
          className="rounded-full bg-primary px-8 py-3 text-sm font-semibold text-primary-foreground shadow-sm transition-[background-color,transform] duration-150 ease-out hover:bg-primary/90 active:scale-[0.97]"
        >
          Get started
        </button>
      </div>
      ...
      <div className="rounded-2xl border border-black/[0.05] bg-white/60 p-5 shadow-[0_1px_3px_rgba(0,0,0,0.02)] backdrop-blur-sm dark:border-border dark:bg-card dark:shadow-none">
        ...
        <div className="divide-y divide-black/[0.04] dark:divide-border">
          <div className="flex items-center justify-between py-3">
            <span className="text-sm text-muted-foreground dark:text-muted-foreground">Format</span>
            <span className="text-sm font-medium text-foreground dark:text-foreground">
              {formatLabels.join(" & ")}
            </span>
          </div>
          <div className="flex items-center justify-between py-3">
            <span className="text-sm text-muted-foreground dark:text-muted-foreground">Edition</span>
            <span className="text-sm font-medium text-foreground dark:text-foreground">
          ...
          {persistedSettings.softcoverPriceMinor && selectedFormats.has("softcover") ? (
            <div className="flex items-center justify-between py-3">
              <span className="text-sm text-muted-foreground dark:text-muted-foreground">Softcover price</span>
              <span className="text-sm font-medium text-foreground dark:text-foreground">
          ...
          {persistedSettings.hardcoverPriceMinor && selectedFormats.has("hardcover") ? (
            <div className="flex items-center justify-between py-3">
              <span className="text-sm text-muted-foreground dark:text-muted-foreground">Hardcover price</span>
              <span className="text-sm font-medium text-foreground dark:text-foreground">
          ...
          {persistedSettings.isbn && (
            <div className="flex items-center justify-between py-3">
              <span className="text-sm text-muted-foreground dark:text-muted-foreground">ISBN</span>
