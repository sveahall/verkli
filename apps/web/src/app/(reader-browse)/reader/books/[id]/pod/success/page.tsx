...
import { confirmStripePodPurchase } from "@/lib/payments/pod";

type PodOutcome = "success" | "pending" | "failed";

export default async function PodSuccessPage({
  params,
  searchParams,
}: {
  params: Promise<{ id: string }>;
  searchParams: Promise<{ pod_order_id?: string; session_id?: string }>;
}) {
  ...
  let outcome: PodOutcome = "pending";
  ...
  if (podOrderId && sessionId) {
    ...
    if (!user) {
      ...
      try {
        const ok = await confirmStripePodPurchase({
          podOrderId,
          sessionId,
          userId: user.id,
          bookId,
      ...
      <section className="w-full rounded-2xl border border-black/10 bg-white p-6 text-slate-900 shadow-sm dark:border-white/10 dark:bg-[#0f1115] dark:text-white">
        ...
        {outcome === "success" ? (
          <p className="mt-3 text-sm text-emerald-700 dark:text-emerald-300">
            Payment verified. Your physical copy is on its way! We will print and ship it shortly.
          </p>
        ) : null}

        {outcome === "failed" ? (
          <p className="mt-3 text-sm text-rose-700 dark:text-rose-300">
            We could not verify the payment. You can try again or contact support.
          </p>
        ) : null}
