...
export async function GET() {
  ...
  try {
    ...
    if (error) throw new Error("Could not read preferences");

    // Use the session client and an explicit owner filter on every page. Keyset
    // pagination avoids the default row cap silently truncating the download.
    async function rows(table: "bookmarks" | "readings" | "listening_positions") {
      ...
      while (true) {
        ...
        if (readError || !data) throw new Error(`Could not read ${table}`);
