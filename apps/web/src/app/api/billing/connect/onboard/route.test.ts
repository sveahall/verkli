...
describe("Stripe onboarding response negotiation", () => {
  ...
  it("preserves the per-user rate limiter before creating an account", async () => {
    mocks.check.mockResolvedValue({ allowed: false });
    expect((await POST(request("text/html"))).status).toBe(429);
    expect(mocks.check).toHaveBeenCalledWith("author-1");
