...
export async function GET(
  ...
  const importJobs = (importRows ?? []).map((r: ImportRow) => {
    ...
    const chapterCount =
      typeof resultMeta.chapterCount === "number" ? resultMeta.chapterCount : null;
    const frontMatterCount =
      typeof resultMeta.frontMatterCount === "number" ? resultMeta.frontMatterCount : null;
    const warnings = Array.isArray(resultMeta.warnings)
      ? resultMeta.warnings.filter((item): item is string => typeof item === "string")
      : [];
    ...
    return {
      ...
      meta: {
        ...
        warnings,
        titleSet:
          typeof resultMeta.titleSet === "boolean" ? resultMeta.titleSet : null,
        bookTitle:
          typeof resultMeta.bookTitle === "string" ? resultMeta.bookTitle : null,
        languageCode:
          typeof resultMeta.languageCode === "string" ? resultMeta.languageCode : null,
        detectedLanguage:
          typeof resultMeta.detectedLanguage === "string"
            ? resultMeta.detectedLanguage
            : null,
        mode: typeof resultMeta.mode === "string" ? resultMeta.mode : null,
      },
