...
const limiter = createPerUserRateLimiter({ maxPerMinute: 2, windowMs: 60_000 });
...
export async function POST(request: Request, context: Context) {
  ...
  try {
    ...
    const output: TranslationQualityRecord = {
      formatVersion: 1, scope: "sample", sourceVersionId, targetVersionId: null, sourceHash,
    ...
    if (jobId) {
      try {
        const { error: saveError } = await admin.from("ai_jobs").update({ status: "failed", error: "The review could not be completed. No quality decision was made.", finished_at: new Date().toISOString() }).eq("id", jobId).eq("user_id", user.id);
