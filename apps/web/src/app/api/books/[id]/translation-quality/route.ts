...
async function authorize(context: Context) {
  ...
  const { data: book, error } = await supabase.from("books")
    .select("id, author_id, original_language, language").eq("id", bookId).maybeSingle();
  if (error) {
...
export async function POST(request: Request, context: Context) {
  ...
  if (!isTranslationsEnabled()) return failure("Translation is currently turned off. Your manuscript has not changed.", 503);
  ...
  try {
    ...
    const { data: chapters, error } = await supabase.from("chapters").select("id, title, content")
      .eq("book_id", bookId).eq("book_version_id", source.sourceVersionId).order("order", { ascending: true }).limit(3);
    ...
    const result = await translateWithQuality({ texts: [originalText], sourceLanguage: source.sourceLanguage, targetLanguage, authorGuidance, signal: request.signal,
      onUsage: async (receipt) => {
        ...
        receiptWrite = receiptWrite.then(async () => {
          const { data, error } = await admin.from("ai_jobs").update({ output: { usageReceipts } as unknown as Json }).eq("id", jobId!).eq("user_id", user.id).select("id").maybeSingle();
    ...
    if (jobId) {
      try {
        const { error: saveError } = await admin.from("ai_jobs").update({ status: "failed", output: { usageReceipts, modelStarted, status: "failed" } as unknown as Json, error: "The review could not be completed. No quality decision was made.", finished_at: new Date().toISOString() }).eq("id", jobId).eq("user_id", user.id);
...
export async function GET(request: Request, context: Context) {
  ...
  let query = supabase.from("ai_jobs").select("id, status, created_at, output, error")
  ...
  const jobs = await Promise.all((data ?? []).map(async (row) => {
    ...
    if (output?.formatVersion === 1 && output.scope !== "sample" && output.targetHash && output.targetVersionId) {
      ...
      const [source, target] = await Promise.all([
        supabase.from("chapters").select("id, title, content, order").eq("book_id", bookId).eq("book_version_id", output.sourceVersionId).order("order", { ascending: true }),
        supabase.from("chapters").select("title, content, order").eq("book_id", bookId).eq("book_version_id", output.targetVersionId).order("order", { ascending: true }),
      ]);
    ...
    return { id: row.id, status: row.status, createdAt: row.created_at, output, error: row.error, stale };
  }));
