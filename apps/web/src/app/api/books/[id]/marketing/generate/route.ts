...
export async function POST(
  ...
  let copy;
  try {
    copy = await generateLaunchCopy({ title: book.title, description: book.description, language, channel, meter: { userId: user.id, pipeline: "marketing", bookId } });
  } catch (error) {
    ...
    return apiError(code, code === "MARKETING_AI_UNAVAILABLE" ? 503 : 502);
  }
