...
export async function POST(
  ...
  // Refuse before any money moves. This sits after the demo guard (the pitch
  // façade never reaches a real TTS provider, so it must not require a voice)
  // and before the Stripe redemption claim below, so a misconfigured
  // deployment cannot burn an author's one-time paid session on a job that
  // was always going to fail at the provider.
  ...
  // Voice was resolved (and required) before the Stripe claim above.
