...
/**
 * Cmd+Shift+V failover. Listens for the BACKUP_VIDEO_EVENT and renders a
 * fullscreen overlay playing /demo-assets/backup-video.mp4. Click anywhere
 * to dismiss.
 *
 * The video file is intentionally optional — the file may not exist on
 * disk yet (the user records it via QuickTime and drops it in manually,
 * same flow as covers/audio/trailer). When the file is missing the
 * <video> element renders an error state and clicking still dismisses.
 */
...
export default function BackupVideoOverlay({ enabled = false }: { enabled?: boolean }) {
  ...
  // Esc key also dismisses (in addition to click) so a stuck demo can
  // recover even when the mouse is unresponsive.
  useEffect(() => {
    if (!open) return;
