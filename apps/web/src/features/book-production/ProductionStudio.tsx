...
export default function ProductionStudio({ initialSettings, initialArtwork = {}, initialSavedSettings = initialSettings, recoveryNotice, onDraftChange, chapters, localPreview = false, localNotice, onSave, onUpload, onBuild, onOpenWriting }: ProductionStudioProps) {
  ...
  function change(patch: Partial<ProductionSettings>) { updateDraft({ ...draftRef.current.settings, ...patch }); setProof(null); setError(null); }
  async function save() {
    if (!onSave || saveLock.current || !validation.success) return;
    saveLock.current = true; setSaving(true); setError(null);
    try { await onSave(validation.data, artwork); if (!mounted.current) return; draftRef.current = { ...draftRef.current, savedSettings: validation.data, recoveryNotice: undefined }; onDraftChange?.(draftRef.current); setSavedSnapshot(snapshot); }
  ...
  return <section className={styles.studio} aria-label="Book production studio">
    ...
    <div role="tabpanel" id={`production-panel-${tab}`} aria-labelledby={`production-tab-${tab}`}>
      {tab === "cover" && <ProductionCover settings={settings} artwork={artwork} onChange={change} onUpload={upload} onRemove={(side) => { const next = { ...draftRef.current.artwork }; delete next[side]; updateDraft({ ...settings, cover: { ...settings.cover, [side === "front" ? "frontPath" : "backPath"]: null } }, next); setProof(null); setError(null); }} uploading={uploading} />}
