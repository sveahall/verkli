...
import TeamBriefing from "@/features/ai-team/TeamBriefing";
import { buildTeamBriefing, inSentence, type BriefingEvent } from "@/features/ai-team/briefing";
...
type HomeWorkspaceProps = {
  /** Personal welcome, picked on the server so both renders agree. */
  greeting: string;
...
export default function HomeWorkspace({
  ...
  const briefing = useMemo(() => {
    const latest = (type: DashboardActivity["type"]): BriefingEvent | undefined => {
      const event = activity.find((item) => item.type === type);
      return event && { label: event.label, bookTitle: event.detail, when: formatRelativeTime(event.timestamp) };
    };
    return buildTeamBriefing({
      book: primaryBook && { id: primaryBook.id, title: primaryBook.title, lastEdited: formatUpdatedAt(primaryBook.updatedAt) },
      sales: stats.sales > 0 ? `${formatCompactNumber(stats.sales)} ${stats.salesCurrency ?? "SEK"}` : null,
      topCountry: countrySales[0] ? countryName(countrySales[0].country) : null,
      readers: stats.readers,
      comments: stats.comments,
      reviews: stats.reviews,
      latest: { translation: latest("translation"), audiobook: latest("audiobook"), publish: latest("publish") },
      enabled: { edith: true, alma: getTranslationsEnabled(), august: getAudiobookEnabled(), stella: getMarketingEnabled(), ernst: true },
    });
  }, [activity, countrySales, primaryBook, stats]);
  ...
  return (
    <>
      <WorkspaceLayout
        header={
          <header>
            <h1 className="author-page-title">{greeting}</h1>
          </header>
        }
        headerRight={<WorkspaceHeaderActions />}
        main={
          <div className="space-y-8">
            {/* One obvious next step, before anything else asks for attention. */}
            <section className="flex flex-col gap-6 rounded-[22px] border border-border bg-card p-6 sm:flex-row sm:items-center sm:justify-between sm:p-7" aria-label="Continue your story">
              <div className="flex min-w-0 items-start gap-4">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-2 text-sm leading-relaxed text-muted-foreground">{primaryBook ? `Last edited ${inSentence(formatUpdatedAt(primaryBook.updatedAt))}.` : "Start with a new book or import a manuscript."}</p>
                </div>
              </div>
              <div className="flex shrink-0 flex-wrap items-center gap-2.5">
                {primaryBook ? <>
                  <Button type="button" variant="secondary" onClick={openCreateDialog} className="min-h-11 rounded-full"><Plus size={17} aria-hidden="true" />New book</Button>
                  <Link href={`/author/books/${primaryBook.id}`} className="inline-flex min-h-11 items-center justify-between gap-5 rounded-full bg-primary px-5 py-3 text-sm font-medium text-primary-foreground transition-colors hover:bg-primary/90">Open editor <ArrowUpRight size={17} aria-hidden="true" /></Link>
                </> : <Button onClick={openCreateDialog} className="min-h-11 rounded-full">Start your first book <Plus size={17} aria-hidden="true" /></Button>}
              </div>
            </section>

            {aiEnabled ? <TeamBriefing briefing={briefing} onCreateBook={openCreateDialog} /> : <>
              <section className="flex flex-wrap items-center gap-2.5" aria-label="Quick actions">
                <Link
                  href={resolveCommandHref("translate-book", { bookId: primaryBook?.id ?? null })}
                  aria-label={primaryBook ? `Translate ${primaryBook.title}` : "Choose a book to translate"}
                  className="inline-flex h-11 items-center gap-2 rounded-full border border-border bg-card px-4 text-[14px] font-medium text-muted-foreground transition hover:bg-background dark:text-foreground dark:hover:bg-accent"
                >
                  <Languages className="h-4 w-4 text-accent-foreground" aria-hidden="true" />
                  Translate book
                </Link>
                <Link
                  href={resolveCommandHref("generate-audiobook", { bookId: primaryBook?.id ?? null })}
              ...
              {/* Without the team there is nobody to relay the numbers, so they stay as cards. */}
