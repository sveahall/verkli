...
function EditionEditor({ scope: inputScope, adapter, manuscript, editionLabel }: Props) {
  ...
  useEffect(() => {
    ...
    adapter.load(scope).then((value) => {
      ...
      setSnapshot(loaded); setRules(loaded.rules); setLoading(false); setError(null); setConflict(false);
    }).catch(() => {
      if (!ignore) { setLoading(false); setError("Rules could not be loaded. Please retry before editing."); }
    });
  ...
  const status = loading ? "Loading rules…" : saving ? "Saving in this demo…" : conflict ? "Another change was saved. Your draft is still here." : error ? "Not saved." : dirty ? "Unsaved changes" : snapshot?.revision ? "Saved in this demo session only" : "No saved rules for this edition";

  async function save() {
    if (!snapshot || !validation.success || saving || conflict) return;
  ...
  function loadLatest() {
    setLoading(true); setError(null); setConflict(false); setReload((value) => value + 1);
  }
  ...
  return <section aria-label="Pronunciation editor" className="space-y-6">
    ...
    <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border bg-background px-4 py-3">
      ...
      <button type="button" className={`${button} bg-primary text-primary-foreground hover:bg-primary/90`} onClick={() => void save()} disabled={loading || saving || !dirty || !validation.success || conflict || !snapshot}><Save aria-hidden="true" size={16} />Save in demo</button>
    </div>
    {error && <div role="alert" className="rounded-xl border border-destructive/40 bg-destructive/5 p-4 text-sm"><p>{error}</p>{!snapshot && <button className={`${button} mt-3`} onClick={loadLatest}>Retry loading rules</button>}</div>}
    {conflict && <div role="alert" className="rounded-xl border border-amber-400/50 bg-amber-50 p-4 text-sm text-stone-900"><p>This edition has a newer saved version. Your draft has not overwritten it.</p><p className="mt-1">Loading the latest rules replaces your draft. Copy any changes you want to keep first.</p><button className={`${button} mt-3 bg-white`} onClick={loadLatest}><RotateCcw aria-hidden="true" size={16} />Load latest and replace draft</button></div>}
    {!loading && snapshot && <div className="grid items-start gap-8 lg:grid-cols-[1fr_0.95fr]">
      <div className="space-y-4">
        ...
        {rules.map((rule, index) => <div key={index} className="rounded-xl border border-border bg-background p-4">
          <div className="mb-3 flex items-center justify-between"><span className="text-xs font-medium text-muted-foreground">RULE {String(index + 1).padStart(2, "0")}</span><button className="flex min-h-11 min-w-11 items-center justify-center rounded-lg hover:bg-muted disabled:opacity-50" disabled={saving} aria-label={`Remove rule ${index + 1}`} onClick={() => { setRules((current) => current.filter((_, i) => i !== index)); setError(null); }}><Trash2 aria-hidden="true" size={16} /></button></div>
          <div className="grid gap-3 sm:grid-cols-[1fr_auto_1fr]">
            <label className="space-y-2 text-xs font-medium">Written form<input aria-label={`Written form ${index + 1}`} className={input} value={rule.word} maxLength={120} disabled={saving} onChange={(event) => edit(index, "word", event.target.value)} placeholder="Mira" /></label>
            <ArrowRight aria-hidden="true" size={16} className="hidden self-end mb-3 text-muted-foreground sm:block" />
            <label className="space-y-2 text-xs font-medium">Spoken as<input aria-label={`Spoken as ${index + 1}`} className={input} value={rule.spokenAs} maxLength={200} disabled={saving} onChange={(event) => edit(index, "spokenAs", event.target.value)} placeholder="Mee-ra" /></label>
          </div>
        </div>)}
        {!validation.success && <p role="alert" className="text-sm text-destructive">{validation.error.issues[0]?.message}</p>}
        <button className={`${button} w-full`} disabled={saving || rules.length >= PROPOSED_MAX_PRONUNCIATION_RULES} onClick={() => setRules((current) => [...current, { word: "", spokenAs: "" }])}><Plus aria-hidden="true" size={16} />Add a pronunciation rule</button>
