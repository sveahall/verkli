...
function ScopedExport({ bookId, editionId, endpoint = `/api/author/books/${bookId}/audiobook/full-export`, synthetic = false }: { bookId: string; editionId: string; endpoint?: string; synthetic?: boolean }) {
  ...
  return <section aria-label="Full-book audio export" className="mx-auto max-w-3xl space-y-6">
    ...
    {synthetic && <p className="rounded-xl border border-border bg-muted/30 p-4 text-sm">Synthetic demonstration: generated tones and simulated account/storage data. This does not test real private audio or paid speech generation.</p>}
    ...
    {preview && <>
      ...
      <aside aria-label="Export capacity" className="rounded-xl bg-muted/30 p-4 text-sm leading-6">Up to 500 chapters, 128 MiB per source and 2 GiB total source audio. Decoded audio is limited to 8 GiB (about 24 hours at 48 kHz mono). The current output limit is {Math.floor(preview.maxOutputBytes / 1024 / 1024)} MiB, with a one-hour processing deadline. These are technical capacity limits.</aside>
