...
import { createContext, useCallback, useContext, useEffect, useRef, useState, useSyncExternalStore, type CSSProperties, type ReactNode, type RefObject, type KeyboardEvent } from "react";
...
export function AuthorStudioExperience() {
  ...
  return <div id="studio" ref={studioRef} className={styles.experience}>
    ...
    <div className={styles.studioLabel}><span><span className={styles.statusDot} />ONE MANUSCRIPT. EVERY POSSIBILITY.</span><span>Go ahead. Make it yours. <ArrowRight size={15} /></span></div>
    <div className={styles.studio} onPointerDownCapture={(event) => { if (!(event.target instanceof Element && event.target.closest("[data-tour-control]"))) setTour(false); }} onKeyDownCapture={(event) => { if (!(event.target instanceof Element && event.target.closest("[data-tour-control]"))) setTour(false); }}>
      <div className={styles.chrome}>
        ...
        <div className={styles.chromeRight}><span className={styles.sampleBadge}>INTERACTIVE SAMPLE</span>{!reduceMotion && <button data-tour-control type="button" aria-label={tour ? "Pause tour" : "Play tour"} onClick={() => { story.pause(); setTour((value) => !value); }}>{tour && !reduceMotion ? <Pause size={13} /> : <Play size={13} />}<span>{tour ? "Pause tour" : "Play tour"}</span></button>}</div>
      </div>
      ...
      <div id="studio-panel" role="tabpanel" aria-labelledby={`studio-tab-${stage}`} className={styles.panel}>
        <AnimatePresence initial={false} mode="wait">
          <motion.div key={stage} initial={{ opacity: 0, y: reduceMotion ? 0 : 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: reduceMotion ? 0 : 0.16 }} className={styles.panelContent}>
            {stage === 0 && <div className={styles.writeLayout}>
              ...
              <div className={styles.paper}>
                <div className={styles.paperToolbar}><span>Chapter 01 <ChevronRight size={12} /> The first page</span><span><span className={styles.savedDot} />Local preview</span></div>
              ...
              <aside className={styles.assistant} aria-label="Writing assistant sample">
                <div className={styles.assistantHeading}><div className={styles.sparkIcon}><Sparkles size={19} /></div><div><strong>A fresh perspective.</strong><span>Still unmistakably you.</span></div></div>
