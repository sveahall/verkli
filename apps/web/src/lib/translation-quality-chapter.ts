...
import type { AuthorProfile } from "./ai/translation-quality/types";
...
export async function translateQualityChapter(input: {
  ...
  for (const [batchIndex, batch] of prepared.plan.batches.entries()) {
    ...
    const result = await translateWithQuality({ texts: source, sourceLanguage: input.sourceLanguage, targetLanguage: input.targetLanguage, profile: input.profile });
