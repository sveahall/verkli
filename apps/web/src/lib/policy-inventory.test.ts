...
describe("activePollsIgnoringVisibility", () => {
  it("flags an active poll that anyone can read", () => {
    const visible: PolicyInventoryRow = {
      ...base,
      policyname: "polls_select",
      cmd: "SELECT",
      qual: "(is_active = true) OR (auth.uid() = author_id)",
      with_check: null,
    };
    const gated: PolicyInventoryRow = {
      ...visible,
      policyname: "polls_select_gated",
      qual: "(auth.uid() = author_id) OR (is_active AND can_view_book(book_id, auth.uid()))",
    };
    const ownVote: PolicyInventoryRow = {
      ...base,
      policyname: "poll_votes_select",
      cmd: "SELECT",
      qual: "(auth.uid() = user_id)",
      with_check: null,
    };
    expect(activePollsIgnoringVisibility([visible, gated, ownVote]).map((row) => row.policyname)).toEqual([
      "polls_select",
    ]);
  });
});
