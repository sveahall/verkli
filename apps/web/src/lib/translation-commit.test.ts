...
import { commitReviewedTranslation, reviewedTranslationActivationReady, TranslationOutcomeUnknownError, type TranslationCommitRequest } from "./translation-commit";
...
describe("reviewed translation deployment admission", () => {
  afterEach(() => vi.unstubAllEnvs());
  it.each([undefined, "", "false", "1", "TRUE"])("holds unless explicitly enabled (%s)", (value) => {
    vi.stubEnv("REVIEWED_TRANSLATION_ATOMIC_ENABLED", value);
    expect(reviewedTranslationActivationReady()).toBe(false);
  });
  it("admits only the explicit atomic rollout switch", () => {
    vi.stubEnv("REVIEWED_TRANSLATION_ATOMIC_ENABLED", "true");
    expect(reviewedTranslationActivationReady()).toBe(true);
  });
});
