...
const STUB_CATALOG: CatalogRow[] = [
  { provider: "stripe", role: "reader", plan_key: "plus", price_id: PLUS_READER, is_active: true, interval: "month" as const },
  { provider: "stripe", role: "author", plan_key: "pro", price_id: PRO_AUTHOR, is_active: true, interval: "month" as const },
];
  ...
  describe("resolveRolePlanFromPriceIdsWithCatalog", () => {
    ...
    it("returns single role when multiple price ids match same role (pro wins over plus)", () => {
      const catalogWithBoth: CatalogRow[] = [
        ...STUB_CATALOG,
        { provider: "stripe", role: "author", plan_key: "plus", price_id: "price_author_plus", is_active: true, interval: "month" as const },
      ];
      const result = resolveRolePlanFromPriceIdsWithCatalog(
