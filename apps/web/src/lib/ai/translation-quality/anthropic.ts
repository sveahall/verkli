...
import { MAX_PROFILE_SAMPLE_CHARS, QUALITY_MODEL, QUALITY_RUBRIC_VERSION, type AuthorProfile, type QualityInput, type QualityResult, type QualityIssue, type TokenUsage } from "./types";
...
function caller(client: Anthropic, signal: AbortSignal, onUsage?: (usage: TokenUsage) => void) {
...
export async function createAuthorProfile(input: ProfileInput, onUsage?: (usage: TokenUsage) => void): Promise<AuthorProfile> {
...
export async function translateWithQuality(input: QualityInput): Promise<QualityResult> {
  ...
  const signal = AbortSignal.any([controller.signal, AbortSignal.timeout(PIPELINE_TIMEOUT_MS), ...(input.signal ? [input.signal] : [])]);
  if (signal.aborted) throw new TranslationQualityError("CANCELLED", "Translation quality processing was cancelled or timed out.");
  const call = caller(getClient(), signal);
