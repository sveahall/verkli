...
const { create, reserve, validate } = vi.hoisted(() => ({ create: vi.fn(), reserve: vi.fn(), validate: vi.fn() }));
...
describe("generateLaunchCopy", () => {
  ...
  it("reserves the complete request and output bound before the provider", async () => {
    ...
    expect(reserve).toHaveBeenCalledWith({ userId: "author", pipeline: "marketing", units });
