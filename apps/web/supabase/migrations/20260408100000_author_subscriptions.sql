-- Author subscriptions (relocated to canonical migration path).
--
-- This migration was previously stored at /supabase/migrations/ at the repo
-- root, outside the canonical /apps/web/supabase/migrations/ tree that
-- supabase config.toml points at. The tables defined here are referenced by
-- production code (settings, /api/reader/subscriptions, /api/author/*,
-- Stripe webhook handlers) so the file is production-required and must live
-- on the canonical path.
--
-- All statements use IF NOT EXISTS / DROP IF EXISTS so this is safe to
-- re-apply against an environment where the orphan migration was already
-- run manually.
...
DROP POLICY IF EXISTS "Author manages own plan" ON public.author_subscription_plans;
...
DROP POLICY IF EXISTS "Authenticated can read enabled plans" ON public.author_subscription_plans;
...
DROP POLICY IF EXISTS "Author reads own subscriber list" ON public.author_subscriptions;
